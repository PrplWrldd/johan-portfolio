...
export const ProjectModal: React.FC<ProjectModalProps> = ({ project, onClose }) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Modal Header */}
        <div className="p-6 sm:p-7 border-b border-slate-800 flex items-start justify-between gap-4 bg-slate-950/60 sticky top-0 z-10">
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-1.5">
              <span className="px-2.5 py-0.5 rounded-full text-xs font-mono bg-sky-950 text-sky-300 border border-sky-800">
                {project.category}
              </span>
          ...
          <button
            ...
            className="p-2 rounded-lg bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 focus:outline-none transition-colors cursor-pointer"
        ...
        <div className="p-6 sm:p-7 space-y-6 overflow-y-auto">
          ...
          <div className="p-5 rounded-xl bg-slate-950 border border-slate-800 flex flex-col items-center justify-center text-center">
            <div className="p-3 rounded-xl bg-slate-900 text-sky-400 mb-2 border border-slate-800">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-xl bg-emerald-950/20 border border-emerald-900/30">
              <h5 className="text-xs font-bold uppercase tracking-wider text-emerald-400 font-mono mb-2 flex items-center gap-1.5">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {project.techStack.map((tech, idx) => (
                <span
                  key={idx}
                  className="px-2.5 py-1 rounded-md text-xs font-mono bg-slate-800 text-sky-300 border border-slate-700"
                >
                  {tech}
                </span>
              ))}
          ...
          <div className="flex items-center gap-2.5">
            {project.githubUrl && (
              <a
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-medium bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 transition-colors"
              >
            ...
            {project.liveDemoUrl && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-medium bg-sky-600 hover:bg-sky-500 text-white transition-colors"
              >
