...
export const Projects: React.FC = () => {
  ...
  const projectIcons: Record<string, React.ReactNode> = {
    maqam: <MapPin className="w-6 h-6 text-emerald-400" />,
    'restaurant-pos': <Utensils className="w-6 h-6 text-sky-400" />,
    'networth-3d': <Box className="w-6 h-6 text-indigo-400" />,
    'iium-bus-tracker': <Bus className="w-6 h-6 text-pink-400" />,
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {filteredProjects.map((project) => (
            <article
              ...
              className="card-govtech rounded-2xl p-6 sm:p-7 flex flex-col justify-between group hover:border-sky-500/40 transition-all"
            >
              <div>
                ...
                <div className="flex items-start justify-between gap-3 mb-4">
                  <div className="p-3 rounded-xl bg-slate-900 border border-slate-800 group-hover:border-slate-700 transition-colors">
                    {projectIcons[project.id] || <Layers className="w-6 h-6 text-sky-400" />}
                  </div>
                ...
                <div className="flex flex-wrap gap-1.5 mb-6">
                  {project.techStack.map((tech, idx) => (
                    <span
                      key={idx}
                      className="px-2 py-0.5 rounded text-[11px] font-mono bg-slate-900/80 text-slate-300 border border-slate-800"
                    >
                      {tech}
                    </span>
                  ))}
              ...
              <div className="pt-4 border-t border-slate-800/80 flex items-center justify-between gap-3">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 text-xs font-semibold text-sky-400 hover:text-sky-300 cursor-pointer focus:outline-none"
                ...
                <div className="flex items-center gap-2">
                  ...
                  {project.liveDemoUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="p-2 rounded-lg bg-sky-950/80 hover:bg-sky-900 text-sky-300 border border-sky-800/80 transition-colors"
                      title="Live Demo / Prototype (Placeholder)"
                    >
